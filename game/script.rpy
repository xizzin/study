...
label Terminallabel():
    ...
    if any(symbol.isdigit() for symbol in nameinp):
        ...
        init python:
            nameinp = "Ноль"  
    elif nameinp == "Йорган" or nameinp == "Йорган Эт-ал":
    ...
    init python:
        ...
        #в этом случае невозможно сделать передачу в файл, так как ренпай не дает возможности сделать чисто питоновский ввод данных и при этом инит питон не воспринимает ренпай инпут как код. соу, здесь будет пусто *разводит руки*. Оно хотя бы работает ув двух случаях из трех? ы    
    ...
    init python:
        ...
        with open('output.json', 'w') as json_file:
            json.dump(Character1, json_file, indent = 4, ensure_ascii=False)
            #также попыталаст сделать вывод ChoiceChecker, но поскольку он /сначала/ делает весь чистый питон и потом свое, из-за этого или ломается игра (если переводить все append в чистый питон), или ничего не выводится
